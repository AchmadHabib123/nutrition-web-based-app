    ...
    <div class="py-7">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-lg overflow-hidden p-6">
                <div class="flex gap-4">
                    @if($menu->gambar)
                        @if (Str::startsWith($menu->gambar, ['http://', 'https://']))
                            <img src="{{ $menu->gambar }}" alt="{{ $menu->nama }}" class="h-32 w-32 object-cover rounded-xl">
                        @else
                            <img src="{{ asset('storage/' . $menu->gambar) }}" alt="{{ $menu->nama }}" class="h-32 w-32 object-cover rounded-xl">
                        @endif
                    @else
                        <img src="https://via.placeholder.com/150" alt="Default Image" class="h-32 w-32 object-cover rounded-xl">
                    @endif
                    <div class="">
                        <h3 class="text-2xl font-bold">{{ $menu->nama }}</h3>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="flex">
                            <div class="flex h-20 bg-slate-300 items-end rounded-full w-6">
                                <div class="bg-green-500 w-full rounded-full" style="height: {{ $menu->protein }}%;"></div>
                            </div>
                        ...
                        <div class="flex">
                            <div class="flex h-20 bg-slate-300 items-end rounded-full w-6">
                                <div class="bg-yellow-500 w-full rounded-full" style="height: {{ $menu->karbohidrat }}%;"></div>
                            </div>
                            <div class="ml-2 text-gray-700">
                        ...
                        <div class="flex">
                            <div class="flex h-20 bg-slate-300 items-end rounded-full w-6">
                                <div class="bg-purple-500 w-full rounded-full" style="height: {{ $menu->total_lemak }}%;"></div>
                            </div>
                            <div class="ml-2 text-gray-700">
                ...
                <p class="text-gray-500 text-lg mt-4">Tipe Pasien: <strong>{{ $menu->tipe_pasien }}</strong></p>
