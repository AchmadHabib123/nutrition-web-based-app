            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('admin.bahan_makanans.update', $bahanMakanan->id) }}">
                    ...
                    <div>
                        ...
                        <select id="tipe" name="tipe" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                            ...
                            @foreach($tipe_options as $tipe)
                                <option value="{{ $tipe }}" {{ old('tipe', $bahanMakanan->tipe) == $tipe ? 'selected' : '' }}>{{ $tipe }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="nama_makanan" class="block mt-1 w-full" type="text" name="nama_makanan" value="{{ old('nama_makanan', $bahanMakanan->nama_makanan) }}" required />
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="kalori" class="block mt-1 w-full" type="number" name="kalori" value="{{ old('kalori', $bahanMakanan->kalori) }}" required min="0" />
...
<x-app-layout>
            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                <form id="editForm" action="{{ route('admin.logistics.bahan_makanans.update', $bahanMakanan) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama" id="nama" value="{{ $bahanMakanan->nama }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm" required>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div class="mb-4">
                            ...
                            <input type="number" name="protein" id="protein" value="{{ $bahanMakanan->protein }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm" required>
                        </div>
    
                        <div class="mb-4">
                            <label for="karbohidrat" class="block text-gray-700">Karbohidrat (gram)</label>
                            <input type="number" name="karbohidrat" id="karbohidrat" value="{{ $bahanMakanan->karbohidrat }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm" required>
                        </div>
    
                        <div class="mb-4">
                            <label for="total_lemak" class="block text-gray-700">Total Lemak (gram)</label>
                            <input type="number" name="total_lemak" id="total_lemak" value="{{ $bahanMakanan->total_lemak }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm" required>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select name="kategori_bahan_masakan" id="kategori_bahan_masakan" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm">
                                <option value="makanan_pokok" {{ $bahanMakanan->kategori_bahan_masakan == 'makanan_pokok' ? 'selected' : '' }}>Makanan Pokok</option>
                                <option value="lauk" {{ $bahanMakanan->kategori_bahan_masakan == 'lauk' ? 'selected' : '' }}>Lauk Pauk</option>
                                <option value="sayur" {{ $bahanMakanan->kategori_bahan_masakan == 'sayur' ? 'selected' : '' }}>Sayuran</option>
                                <option value="buah" {{ $bahanMakanan->kategori_bahan_masakan == 'buah' ? 'selected' : '' }}>Buah</option>
                                <option value="susu" {{ $bahanMakanan->kategori_bahan_masakan == 'susu' ? 'selected' : '' }}>Susu</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="tipe_pasien" id="tipe_pasien" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm">
                                <option value="VVIP" {{ $bahanMakanan->tipe_pasien == 'VVIP' ? 'selected' : '' }}>VVIP</option>
                                <option value="VIP" {{ $bahanMakanan->tipe_pasien == 'VIP' ? 'selected' : '' }}>VIP</option>
                                <option value="Normal" {{ $bahanMakanan->tipe_pasien == 'Normal' ? 'selected' : '' }}>Normal</option>
                            </select>
                        </div>
                    </div>
                    <div class="mb-4">
    ...
    <script>
        function submitEditForm() {
            // Tampilkan alert
            alert("Berhasil Edit bahan_makanan);
